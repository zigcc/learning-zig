...
我们最初的目标是通过`levelUp`函数将用户的`power`值增加1 。我们已经让代码编译通过，但当我们打印`power`时，它仍然是原始值。虽然有点跳跃，但让我们修改代码，在 `main` 和 `levelUp` 中打印 `user`的地址：
...
新的`name`字段是一个切片，由`len`和`ptr`字段组成。它们与所有其他字段一起按顺序排放。在64位平台上，`len`和`ptr`都将是64位，即8字节。有趣的是`name.ptr`的值：它是指向内存中其他位置的地址。
